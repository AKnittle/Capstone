...
```{r Exploration, echo=FALSE, fig.cap= "Exploration", out.height="100%", out.width="100%", warning=FALSE, comment=FALSE, warning=FALSE}
...
```{r Exploration 2, echo=FALSE, fig.cap= "Exploration 2", out.height="100%", out.width="100%", warning=FALSE, comment=FALSE, warning=FALSE}
...
TODO: What's up with Carbaryl and Diazinon
...
5/6/21
TODO:
Look into CreateTableOne
  Look at differences between groups
  Start Modeling
    Univariate Stat Regression
    Build model; Logistical, LDA, Decision Trees (Conditional Decision Trees, Package Partykit->cpart), Lasso
  Would be worth looking at coorelation between chems 
    Coorelation Matrices
    Use log+1 scale
  Kernel Methods
    Using kernel as a way to finding predictors
    Kernal methods for mixtures
      Bayesian (Biostatistics, multipollutent mixture)
  Weighted Quantile Sum Regression
    David Wheiler Paper (Check the Google Drive)
      Sum score for all chems
        Use the one score for logistic regression
  HW: write up (Set Goals, How we do comparisons, etc.), EDA, Matrix correlation, table 
    Know how we are going to compare models; cross validation, AIC, and others
    Due: 5/11/21
  

```{r Exploration 3, echo=FALSE, fig.cap= "Exploration 3", out.height="100%", out.width="100%", warning=FALSE, comment=FALSE, warning=FALSE}

# Making log transformed values for all chem values 

logDustData = as.data.frame(read.csv('Data/logDustData_Reduced.csv',header=T,sep=','))
# logDustData <- dustData_Reduced
# logDustData[chemList] <- lapply(logDustData[chemList], function(x) log10(x+1))
logDustData <- logDustData[-1]
extraVars <- as.vector(c("SITE", "GENDER", "AGE_REF", "RACEG3"))
logDustData <- select(logDustData,-X)
logDustData <- select(logDustData, -extraVars)
#write.csv(logDustData, "logDustData_Reduced.csv")
