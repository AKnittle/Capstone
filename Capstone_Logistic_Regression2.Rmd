...
```{r Cross Validate Model, echo=FALSE, fig.cap= "Cross Validate Model", out.height="100%", out.width="100%", warning=FALSE, comment=FALSE, warning=FALSE}

# Do Cross Validation

tibble(Model=c("Logistic Model"), Error_Rate = c(error.rater(logDustData, stepwiseModel2, 10)))

```

[A common test of model calibration is the Hosmer-Lemeshow test, which compares the proportion of observed outcomes across quantiles of predicted probabilities. Deciles are often used, but this is arbitrary. The Hosmer-Lemeshow test will provide a p-value, which reflects the probability that your null hypothesis – that there is no difference between the distribution of predicted and observed outcomes across quantiles – is correct (in other words, this is a situation in which a very large p-value should make the analyst quite pleased).](https://www.publichealth.columbia.edu/research/population-health-methods/risk-prediction#Description)
...
#TODO: Risk calibration: make part of the cross validation
...
beforeCross <- cvResult[[2]]
# trainCross <- cvResult[[3]]
validCross <- cvResult[[4]]
# trainCross$decilePredictedVals <- decileBuilder(trainCross$train.rawPredict)
validCross$decilePredictedVals <- decileBuilder(validCross$holdout.rawPredict)

decileBins <- seq(0,1, 0.1)
decileProp <- data.frame()
for(i in decileBins){
  # print(i)
  tempSet <- subset(validCross, decilePredictedVals==i)
...
# Graph Result
# trainCrossPlot <- ggplot(trainCross) +
#   aes(x = decilePredictedVals, fill = Response) +
#   geom_bar(position = "dodge") +
#   scale_fill_hue(direction = -1) +
#   labs(x = "Decile Values", y = "Count") +
#   theme_bw() +
...
validCrossPlot <- ggplot(validCross) +
  aes(x = decilePredictedVals, fill = Response) +
  geom_bar(position = "dodge") +
  scale_fill_hue(direction = -1) +
  labs(x = "Decile Values", y = "Count") +
...
# TODO: Look at quantiles instead
# quantile(validCross$holdout.rawPredict, probs = seq(.1, .9, by = .1))
# New Bins (0.4508003 0.4971952 0.5256168 0.5506360 0.5727034 0.5993392 0.6217922 0.6488835 0.6816872)
ggplot(decileProp) +
  aes(x = Decile, y = props) +
  geom_line(size = 0.5, colour = "#461112") +
  theme_bw() +
  ylim(0L, 1L)
