...
# lda.formula <- formula(CASESTAT ~ DDE_1)
# lda.fit <- lda(lda.formula, data = logDustData)
# lda.fit
# lda.k.fold.validator2(logDustData, "CASESTAT", 10)
# errorRateTable.test <- lda.k.fold.validator2(logDustData, "CASESTAT", 10)
# errorRateTable.test
# test.predValues <-as.numeric(as.character(predict(lda.fit, logDustData)$class))
# 
# rawConfusionBuilder(logDustData$CASESTAT, test.predValues)

# Reattempting with my swiss army cross validator and with the full log data set
lda.formula <- formula(CASESTAT ~ .)
lda.fit <- lda(lda.formula, data = logDustData)
crossValidator.byKPercent(df = logDustData, response = "CASESTAT", formula = lda.formula, family="lda")
...
rocFull <- rocBuilder(train_DustData$CASESTAT, test.predValues, toleranceVec = seq(0,1,0.001))
rocFull[[2]]
rocDF <- rocFull[[1]]
...
From the Mayo Clinic: ["Chemicals. Certain chemicals, such as those used to kill insects and weeds, may increase your risk of developing non-Hodgkin's lymphoma. More research is needed to understand the possible link between pesticides and the development of non-Hodgkin's lymphoma."](https://www.mayoclinic.org/diseases-conditions/non-hodgkins-lymphoma/symptoms-causes/syc-20375680)
